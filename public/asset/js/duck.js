...
const turnMotherDuckAndUpdateDirection = ({ diff, limit }) => {
  ...
  updateData(data.duck, {
    angle: newAngle,
    direction: getDirectionClass(newAngle),
  });
...
const triggerDuckMovement = () => {
  data.interval = setInterval(() => {
    const fullDistance = distanceBetween(
      offsetPosition(data.duck),
      data.cursor
    );
...
const createDuckling = () => {
  ...
  data.ducklingTargets.push({
    el: newDucklingTarget,
    x: 0,
    y: 0,
    timer: null,
    offset: 6,
...
// set up
["click", "mousemove"].forEach((action) =>
  window.addEventListener(action, updateCursorPos)
);
